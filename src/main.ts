...
const bindings = (await connection.query(
  'SELECT * FROM google_docs_binding WHERE entity_name = "poll_answer"'
)) as any[]

async function uploadFile(drive: any, csv: string) {
  const fileMetadata = {
    name: 'noo-export.csv',
...
for (const binding of bindings) {
  ...
  if (entityName === 'poll_answer') {
    ...
    const header = ['', ...questions.map((question: any) => question.text)]

    const results = []

    const userAnswers = answers.reduce((acc: any, answer: any) => {
